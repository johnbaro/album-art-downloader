...
namespace AlbumArtDownloader
{
	...
	public class SearchQueue: INotifyPropertyChanged
	{
		private class NativeMethods
		{
			...
			internal static extern int SetWindowPos(IntPtr pHWND, IntPtr pAfterHWND, int pX, int pY, int pCX, int pCY, int pFlags);
		...
		private void DequeueNextSearchWindow()
		{
			if (Queue.Count > 0)
			{
				while (NumberOfOpenSearchWindows < SimulataneousWindowsAllowed) //Keep dequeueing as long as more simultaneous windows are allowed.
				{
					...
					//Place it behind the first existing search windows
					ArtSearchWindow firstWindow = Application.Current.Windows.OfType<ArtSearchWindow>().FirstOrDefault();
					if (firstWindow != null)
					{
						NativeMethods.SetWindowPos(new WindowInteropHelper(searchWindow).Handle, new WindowInteropHelper(firstWindow).Handle, 0, 0, 0, 0, NativeMethods.SWP_NOACTIVATE | NativeMethods.SWP_NOMOVE | NativeMethods.SWP_NOSIZE);
					}
				}
				if (Queue.Count == 0)
				{
