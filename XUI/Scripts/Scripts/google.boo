...
class GoogleImage(AlbumArtDownloader.Scripts.IScript):
	...
	def Search(artist as string, album as string, results as IScriptResults):
		imagesHtml = GetPage("http://images.google.com/images?gbv=1&q=" + EncodeUrl(artist + " " + album))

		imageMatches = @/(?i)\/imgres\?imgurl=(?<fullSize>[^&]+)[^>]+?&h=(?<height>\d+)&w=(?<width>\d+)[^>]+?&tbnid=(?<tbnid>[^&]+)/.Matches(imagesHtml)
		...
		for i in range(imageMatches.Count):
			...
			results.AddThumb("http://tbn0.google.com/images?q=tbn:${tbnid}", title, width, height, fullSize);
