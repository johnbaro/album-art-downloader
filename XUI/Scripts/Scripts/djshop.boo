...
class DJshop(AlbumArtDownloader.Scripts.IScript):
	...
	def Search(artist as string, album as string, results as IScriptResults):
		...
		matches = Regex("<a href=\"(?<url>[^\"]+)\"[^>]+><img src=\"(?<imageUrlBase>http://download\\.feiyr\\.com/[^\"]+?,)80(?<imageUrlTail>[^\"]+)\" alt=\"(?<title>[^\"]+)\"", RegexOptions.Singleline | RegexOptions.IgnoreCase).Matches(searchResultsHtml)
