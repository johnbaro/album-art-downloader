...
namespace AlbumArtDownloader
{
    ...
    public partial class FileBrowserDetail : System.Windows.Window, INotifyPropertyChanged, IAppWindow
    {
        ...
        {
            Search(mFilePathBox.Text, true, string.Empty);
        }

        // these properties are here instead of configurable checkboxes as in FileBrowser window
        private bool mIncludeSubfolders = true;
        private string mImagePathPatternBoxPathPattern = string.Empty;
        ...
        public void Search(string rootPath, bool includeSubfolders, string imagePathPattern, string filePathPattern)
        {
            ...
            mImagePathPatternBoxPathPattern = imagePathPattern;
        ...
        private void CreateMediaFileSearchThread()
        {
            ...
            mMediaFileSearchThread.Name = "File Browser Detail Search";
        ...
        /// <summary>
        /// Method will go through mAlbums collection and if there is a record for given information
        /// returns its instance, otherwise new Album will be created.
        /// </summary>
        /// <param name="basePath"></param>
        /// <param name="artistName"></param>
        /// <param name="albumName"></param>
        /// <returns></returns>
        private Album findInAlbums(String basePath, String artistName, String albumName)
        {
            Album album = new Album(basePath, artistName, albumName);

            // mAlbums already contains information about given album
            // we have to find this album and return its existing instance
            // so all tracks for given album will be included in result tree
            if (mAlbums.Contains(album))
            {
                foreach (Album existingAlbum in mAlbums)
                {
                    // we find match - return it and break the searching
                    if (existingAlbum.Artist.Equals(artistName) && existingAlbum.Name.Equals(albumName))
                    {
                        album = existingAlbum;
                        break;
                    }
